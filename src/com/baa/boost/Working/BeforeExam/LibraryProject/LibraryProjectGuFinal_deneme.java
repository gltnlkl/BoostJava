...
public class LibraryProjectGuFinal_deneme {
	...
	//
	// Book myBook = new Book(bookList[0], bookList[1], bookList[2], bookList[3],
	// bookList[4], bookList[5],
	// bookList[6], bookList[7]);
	//
	// return myBook;
	// }
	...
	public static void main(String[] args) {
		...
		// List<Book> mapList = listBook.stream().filter(book ->
		// book.getAuthor().startsWith("J.K. Rowling"))
		...
		// List<String> mapList =
		// filterMap.stream().map(FilterStudent::getAdi).filter(adi ->
		// adi.startsWith("Ahmet"))
	...
	private static void fileWrite(String path) {
		...
		try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(path, false))) { // orjınal dosyayı
			...
			bufferedWriter.write(
					"book_id;isbn;authors;original_publication_year;original_title;title;language_code;average_rating"); // ılk
																															// satır
			for (Book book : listBook) {
